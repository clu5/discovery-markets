        ...
        while i < tau or curr_max <= metric:
            ...
            if run_seq:
                ...
                print(
                    ...
                    + ";"
                    + new_col_lst[candidate_id].join_path.join_path[1].col,
                )
                ...
                total_queries += 1
                fout.write(
                    str(max(curr_max, curr_max_grp)) + " " + str(total_queries) + "\n"
                )
            ...
            while cl_iter < k:
                ...
                while samp_iter < count:
                    ...
                    if not curr_samp in queried_cand.keys():
                        total_queries += 1
                    tmp_metric = max(
                        oracle.train_classifier(merged_df, class_attr), metric
                    )
                    ...
                    if tmp_metric > curr_max:
                        ...
                        max_candidate = merged_df
                        max_file = (
                            curr_samp.join_path.join_path[1].tbl
                            + ";"
                            + curr_samp.join_path.join_path[1].col
                        )
                        print(
                            "metric", tmp_metric, curr_samp.join_path.join_path[1].tbl
                        )  # ,corr)
                        print(merged_df)
                        max_jc = curr_samp.loc
                    #    irregularity_count+=1
                    fout.write(
                        str(max(curr_max, curr_max_grp))
                        + " "
                        + str(total_queries)
                        + "\n"
                    )
