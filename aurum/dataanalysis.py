...
def get_numerical_signature(values, S):
    ...
    # Learn kernel
    kde = KernelDensity(kernel=C.kd["kernel"], bandwidth=C.kd["bandwidth"]).fit(X)
...
def get_sim_items_ks(key, ncol_dist):
    sim_columns = []
    sim_vector = get_sim_vector_numerical(key, ncol_dist, "ks")
...
def get_dist(data_list, method):
    ...
    if method == "kd":
        ...
        svmachine = svm.OneClassSVM(
            nu=C.odsvm["nu"], kernel=C.odsvm["kernel"], gamma=C.odsvm["gamma"]
        )
    ...
    for key, value in tcol_dist.items():
        if value_to_compare is not "" and value is not "":
            try:
                ...
                print("No sim for (" + str(column) + "" + str(key) + ")")
                vt[key] = -1
    return vt
