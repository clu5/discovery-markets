...
from aurum.api.apiutils import Hit
...
class StoreHandler:
    ...
    def __init__(self):
        ...
        client = Elasticsearch(
            [{"host": c.db_host, "port": c.db_port, "scheme": "http"}]
        )
    ...
    def get_all_fields(self):
        ...
        while remaining > 0:
            hits = res["hits"]["hits"]
            for h in hits:
                id_source_and_file_name = (
                    h["_id"],
                    h["_source"]["dbName"],
                    h["_source"]["sourceName"],
                    h["_source"]["columnName"],
                    h["_source"]["totalValues"],
    ...
    def get_all_fields_with(self, attrs):
        ...
        body = {"query": {"match_all": {}}}
        res = client.search(
            index="profile", body=body, scroll="10m", filter_path=filter_path
        )
        scroll_id = res["_scroll_id"]
        remaining = res["hits"]["total"]["value"]
        while remaining > 0:
            ...
            for h in hits:
                ...
                remaining -= 1
            res = client.scroll(
                scroll="5m", scroll_id=scroll_id, filter_path=filter_path
            )
            scroll_id = res["_scroll_id"]  # update the scroll_id
        client.clear_scroll(scroll_id=scroll_id)
    ...
    def suggest_schema(self, suggestion_string, max_hits=5):
        ...
        res_completion = [
            ((el["_source"]["columnName"], el["_source"]["sourceName"]), el["_score"])
            for el in res["suggest"]["schema-completion"][0]["options"]
        ]
    ...
    def get_all_docs_from_text_with_idx_id(self, doc_id):
        ...
        res = client.search(
            ...
            body=body,
            scroll="10m",
            filter_path=["_scroll_id", "hits.hits._id", "hits.total"],
        )
        scroll_id = res["_scroll_id"]
        remaining = res["hits"]["total"]["value"]
        while remaining > 0:
            ...
            res = client.scroll(
                scroll="5m",
                scroll_id=scroll_id,
                filter_path=["_scroll_id", "hits.hits._id", "hits.hits._source.id"],
            )
            scroll_id = res["_scroll_id"]  # update the scroll_id
        client.clear_scroll(scroll_id=scroll_id)
    ...
    def add_annotation(
        ...
        for tag in tags:
            mapped_tags.append(
                {"author": author, "creation_date": timestamp, "tag": tag}
            )
        ...
        hit = MDHit(
            res["_id"], author, md_class, text, source, target["id"], target["type"]
        )
        return hit
    ...
    def add_comment(self, author: str, text: str, md_id: str):
        ...
        body = {"author": author, "text": text, "creation_date": timestamp}

        res = client.create(
            index="metadata", doc_type="comment", body=body, parent=md_id
        )
                    ...
                    "should": [
                        {"match": {"text": keywords}},
                        {
                            "nested": {
                                ...
                                "query": {
                                    "bool": {
                                        "should": [{"match": {"tags.tag": keywords}}]
                                    }
                                },
                            }
                        },
    ...
    def add_tags(self, author: str, tags: list, md_id: str):
        ...
        new_tags = []
        for tag in tags:
            new_tags.append({"author": author, "creation_date": timestamp, "tag": tag})
        new_tags.extend(source["tags"])

        body = {"doc": {"updated_date": timestamp, "tags": new_tags}}
        res = client.update(
            index="metadata", doc_type="annotation", id=md_id, body=body
        )
        return MDHit(
            res["_id"],
            author,
    ...
    def get_metadata(
        self, nid: str = None, relation: str = None, nid_is_source: bool = True
    ):
        """
        :param nid: node id
        ...
        match_target_id = {
            "nested": {
                "path": "target",
                "query": {"bool": {"should": [{"term": {"target.id": nid}}]}},
            }
        }
        ...
        if nid is None:
            ...
            body = {"query": {"bool": {"should": [match_source_id, match_target_id]}}}
        else:
    ...
    def get_comments(self, md_id: str):
        ...
        body = {
            "query": {
                "has_parent": {
                    ...
                    "query": {"bool": {"should": [{"term": {"_id": md_id}}]}},
                }
            }
        }
        ...
        for md in res["hits"]["hits"]:
            yield MDComment(
                md["_id"], md["_source"]["author"], md["_source"]["text"], md["_parent"]
            )
