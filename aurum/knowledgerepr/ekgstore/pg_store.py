...
class PGStore:
    ...
    def init_schema(self):
        ...
        self.cur.execute(
            "CREATE TABLE edges (source_node_id integer, "
    ...
    def new_node(
        self, node_id=None, node_name=None, table_name=None, unique_ratio=None
    ):
        self.cur.execute(
            "insert into nodes (node_id, node_name, table_name, unique_ratio) "
            "values (%s, %s, %s, %s)",
            (node_id, node_name, table_name, unique_ratio),
        )

    def new_edge(
        self, source_node_id=None, target_node_id=None, relation_type=None, weight=None
    ):
        self.cur.execute(
    ...
    def connected_to(self, nid, relation_type):
        self.cur.execute(
            "select target_node_id from edges where source_node_id = %s and relation_type = %s",
            (nid, relation_type),
        )
        results = []
...
if __name__ == "__main__":
    ...
    for src_id, tgt_id in random_g.edges():
        ...
        try:
            ...
            store.rollback()
        store.new_edge(
            source_node_id=src_id, target_node_id=tgt_id, relation_type=1, weight=0.55
        )
        store.commit()
