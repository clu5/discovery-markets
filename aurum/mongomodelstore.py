...
def get_numerical_signatures():
    cursor = modeldb.find(
        {"type": "N"}, {"filename": 1, "column": 1, "signature": 1, "_id": 0}
    )
    results = []
    for el in cursor:
...
def get_textual_signatures():
    cursor = modeldb.find(
        {"type": "T"}, {"filename": 1, "column": 1, "signature": 1, "_id": 0}
    )
    results = []
    for el in cursor:
        key = (el["filename"], el["column"])
...
def peek_values(concept, num):
    ...
    res = modeldb.find({"key": key}, {"type": 1, "n_data": 1, "t_data": 1})
    values = None
    type_col = None
    n_data = None
    t_data = None
...
def get_fields_from_concept(concept, arg1, arg2):
    ...
    res = modeldb.find({"key": key}, {arg1: 1, arg2: 1, "_id": 0})
...
def search_keyword(keyword):
    ...
    res = modeldb.find(
        {"$text": {"$search": keyword}}, {"filename": 1, "column": 1, "_id": 0}
    )
